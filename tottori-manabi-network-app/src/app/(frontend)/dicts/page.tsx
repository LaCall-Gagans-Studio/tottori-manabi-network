            ...
            dict.hasPage ? (
              ...
              >
                <div className="h-full z-10 bg-[#f8fdee] pr-3 rounded-lg rounded-r-lg flex relative duration-300 group-hover:-translate-x-1 lg:group-hover:-translate-x-8 transition-all">
                  <div
                    className={
                      `absolute -top-2 lg:-top-5 -left-2 lg:-left-5 w-10 lg:w-12 h-10 lg:h-12 flex items-center justify-center 
                      text-white text-base lg:text-lg font-medium rounded-full shadow-md ring-2 ring-ws-white ring-offset-2 
                      transition-colors duration-200 ` +
                      (dict.tags?.find((tag) => tag.name?.includes('東部'))
                        ? 'bg-ws-primary group-hover:bg-lime-500'
                        : dict.tags?.find((tag) => tag.name?.includes('中部'))
                          ? 'bg-ws-secondary group-hover:bg-pink-500'
                          : dict.tags?.find((tag) => tag.name?.includes('西部'))
                            ? 'bg-ws-tertiary group-hover:bg-blue-500'
                            : 'bg-gray-300')
                    }
                  >
                    {dict.tags?.find((tag) => tag.name?.includes('東部'))
                  ...
                  <div
                    className={`absolute bottom-0 left-0 px-2 py-0.5 bg-ws-primary text-white text-xs lg:text-lg rounded-se-lg  ${
